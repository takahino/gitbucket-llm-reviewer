...
- **大量修正PRはバッチ分割で対応(切り捨てない)** — PR全体のdiffが `review.maxDiffChars` を超える場合、ファイル境界を尊重して複数バッチに分割し、バッチ毎に独立してLLMへ送信します(切り捨てて見落とすのではなく、全ファイルをレビュー対象にする)。暴走防止のため `review.maxDiffBatches` で1PRあたりのバッチ数上限を設定でき、超過した分のファイルは黙って消さずサマリコメントにファイル名を明記します。
...
review:
  maxDiffChars: 60000    # 1バッチ(1回のLLMリクエスト)あたりのdiff文字数上限(超過分は切り捨てず複数バッチに分割)
  maxDiffBatches: 20     # 1PRあたりの最大バッチ数。超過分は切り捨てず、対象外ファイルとしてサマリに明記
...
2. `ReviewOrchestrator` が `.review.yml` を読み込み(GitBucketのcontents API、失敗時はJGit経由)、diffを計算します(JGitによるmerge-base差分をプライマリ、失敗時はコミット単位パッチの連結にフォールバック)。解決した観点(共通+モノレポのパスグループ)が0件(`.review.yml` 未配置・パース失敗・空の `perspectives` 等)の場合はそのPRのレビューをスキップします。
3. diffが `review.maxDiffChars` を超える場合、`DiffBatcher` がファイル境界を尊重して複数バッチに分割します(バッチ数の上限は `review.maxDiffBatches`。超過分のファイルは今回のレビュー対象外としてサマリに明記されます)。以降のステップはバッチ毎に独立して実行され、観点解決・コンテキスト収集・RAG検索もバッチ内の変更ファイルに絞って行われます。
4. `rag.enabled: true` の場合、各バッチのdiffをクエリとしてリポジトリコード(初回は全量、以降は増分)とコーディング規約文書(`.review.yml` の `knowledgeBase`)をベクトル検索し、関連チャンクを「参考情報」として収集します(embeddingサーバー不通時は空扱いでフォールバックし、レビュー自体は継続します)。
5. 各バッチについて、LLMにdiff・観点・リポジトリのファイル一覧・RAG参考情報を渡します。`need_more_context` が返された場合は要求されたファイルを取得(重複除去・サイズ上限あり)し、`review.maxPasses` 回まで再問い合わせします。
6. 全バッチのfindingsとsummaryを結合し、最終的なサマリと指摘事項をMarkdownに整形してPRコメントとして投稿します。レビュー済みのhead SHAを永続化するため、同じコミットが二重にレビューされることはありません。
7. これとは独立して、`PollingService` は各PRについて `MentionReplyOrchestrator` にもメンション応答の確認を依頼します。起動時に解決したBotユーザー名(`gitbucket.botUsername`、空なら `GET /api/v3/user` で自動解決)へのメンションを含む未処理コメントを検知すると、`.review.yml`(あれば)・diff・これまでのコメント履歴・メンションコメント本文を渡してLLMに回答させ、その回答を新規コメントとして投稿します。処理済みの最終コメントIDを永続化するため、同じコメントに二重応答することはありません。`.review.yml` が無いリポジトリでも、この経路は通常のレビューとは独立して常時動作し、メンションコメントの文面そのものを観点として扱います。
